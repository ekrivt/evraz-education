...
class RegistrationTestCase(APITestCase):
    def test_registration(self):
        data={"username":"test_user1","password":"test_pass","email":"test@test.com"}
...
class userProfileTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user=self.client.post('/auth/users/',data={'username':'Sam','password':'SamPass!'})
        # получить веб-токен JSON для вновь созданного пользователя
        response=self.client.post('/auth/jwt/create/',data={'username':'Sam','password':'SamPass!'})
    ...
    def test_userprofile_profile(self):
        profile_data={'description':'Admin Sam','location':'Moscow', 'status':'admin','is_creator':'true',}
